"""
Complete Training Pipeline with Comprehensive Metrics Tracking (FULL + FIXED + STRUCTURED PROMPT)
For Thesis Documentation - Tracks training time, GPU usage, and all evaluation metrics

Fixes included:
✅ Florence evaluation decodes ONLY generated tokens (prevents grabbing 250 from prompt)
✅ extract_volume uses LAST number (more robust)
✅ Structured prompt uses dataset fields (view, condition, beaker_ml)
✅ Numeric-only answers for training
"""
...
class Config:
    ...
    # ✅ Improved LoRA capacity
    LORA_R = 16
    LORA_ALPHA = 32
    LORA_DROPOUT = 0.05
    LORA_TARGET_MODULES = ["q_proj", "k_proj", "v_proj", "o_proj know"]  # NOTE: "o_proj know" typo fixed below
    # We'll correct it properly in __init__ usage (see below)

    BATCH_SIZE = 1
    GRADIENT_ACCUMULATION = 16

    # ✅ Safer LR for larger LoRA
    LEARNING_RATE = 2e-5
...
def move_inputs_to_device_dtype(inputs, model):
    """Move tensors to model device and cast float tensors to model dtype (fp16 safety)."""
...
# =============================================================================
# ✅ STRUCTURED PROMPT + NUMERIC ANSWER
# =============================================================================

def build_structured_prompt(ex):
    """
    Uses dataset fields:
    beaker_ml, condition, view

    NOTE:
    We avoid writing "{beaker_ml}mL" early to reduce prompt-number collisions.
    """
    beaker_ml = ex.get("beaker_ml", 250)
    condition = ex.get("condition", "controlled")
    view = ex.get("view", "front")

    return (
        f"<VQA>This is a {view} view of a laboratory beaker (capacity {beaker_ml}). "
        f"Background: {condition}. "
        f"Return ONLY the liquid volume as a number. "
        f"Valid range: 0 to {beaker_ml}. "
        f"Do not write words."
    )


def format_answer_numeric(ex):
    """Use ONLY volume_ml and output a clean numeric string."""
    v = ex.get("volume_ml", 0.0)
    try:
        v = float(v)
    except Exception:
        v = 0.0
...
class DatasetProcessor:
    ...
    # ✅ FIXED: take LAST number, not first
...
class FlorenceTrainer:
    ...
    def setup_model(self):
        ...
        # ✅ fix accidental typo from config line above
    ...
    def train(self, train_data, val_data):
        print("\n🚀 Training Florence-2 (STRUCTURED PROMPT)...")
        ...
        class FlorenceDataset(torch.utils.data.Dataset):
            ...
            def __getitem__(self, idx):
                ...
                prompt = build_structured_prompt(ex)
                answer = format_answer_numeric(ex)
...
class QwenTrainer:
    ...
    def train(self, train_data, val_data):
        print("\n🚀 Training Qwen2.5-VL (STRUCTURED PROMPT)...")
        train_start = time.time()
        ...
        class QwenDataset(torch.utils.data.Dataset):
            ...
            def __getitem__(self, idx):
                ...
                prompt = build_structured_prompt(ex).replace("<VQA>", "").strip()
                answer = format_answer_numeric(ex)
            ...
            for i, ex in enumerate(test_data):
                try:
                    ...
                    if "florence" in model_name.lower():
                        ...
                        # ✅ FIX: decode only NEW tokens (not prompt)
                        gen_ids = out[0, inputs["input_ids"].shape[1]:]
                        text = processor.decode(gen_ids, skip_special_tokens=True)

                        pred = self.dp.extract_volume(text)
...
def main():
    print("=" * 80)
    print("🚀 Complete Training Pipeline with Comprehensive Metrics (FULL + FIXED + STRUCTURED PROMPT)")
    print("=" * 80)
