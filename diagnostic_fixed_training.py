"""
DIAGNOSTIC VERSION - Fixes Florence's Catastrophic Failure
Key changes:
1. Better prompt engineering
2. Improved label masking
3. Enhanced answer format
4. Temperature control during generation
5. More aggressive regularization
"""
...
class Config:
    ...
    # LoRA - More aggressive for Florence
    ...
    # Different LR for each model
    FLORENCE_LR = 1e-4  # Higher LR for Florence
    QWEN_LR = 3e-5
    WEIGHT_DECAY = 0.05  # More regularization
    
    NUM_EPOCHS = 10  # More epochs
    WARMUP_RATIO = 0.1  # Ratio instead of steps
    MAX_LENGTH = 256
...
class FlorenceTrainer:
    ...
    def setup_model(self):
        print("\n🤖 Loading Florence-2 with FIXED configuration...")
        clear_memory()
    ...
    def train(self, train_data, val_data):
        print("\n🚀 Training Florence-2 with IMPROVED PROMPT...")
        ...
        class FlorenceDataset(torch.utils.data.Dataset):
            ...
            def __getitem__(self, idx):
                ...
                # IMPROVED ANSWER FORMAT - just the number
                answer = str(int(round(gt)))  # Round to nearest integer
                ...
                # Create labels - ONLY supervise the answer
        ...
        args = TrainingArguments(
            ...
            learning_rate=cfg.FLORENCE_LR,  # Higher LR
            ...
            max_grad_norm=1.0,
            label_smoothing_factor=0.0,  # No label smoothing for regression
        )
...
# =============================================================================
# QWEN TRAINER (Same as before - it's working well)
# =============================================================================
    ...
    def train(self, train_data, val_data):
        ...
        class QwenDataset(torch.utils.data.Dataset):
            ...
            def __getitem__(self, idx):
                ...
                if gt is None:
                    gt = 0.0
                answer = str(int(round(gt)))  # Integer answer
            ...
            for i, ex in enumerate(test_data):
                try:
                    ...
                    if "florence" in model_name.lower():
                        ...
                        out = model.generate(
                            **inputs, 
                            max_new_tokens=10,  # Shorter - just need a number
                            do_sample=False,
                            num_beams=1,
                        )
...
def main():
    print("=" * 80)
    print("🚀 DIAGNOSTIC VERSION - Fixing Florence's Catastrophic Failure")
    print("=" * 80)
    ...
    print("\n" + "=" * 80)
    print("EVALUATION WITH FIXED PROMPTS")
    print("=" * 80)
